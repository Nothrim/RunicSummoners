...
public class BaseUnit {
    //basic variables
    Player owner;
    public final static float MELEE_RANGE=45;
    ...
    public int getWorldX(float scale){return (int)(positionX/scale);}
    ...
    {
        return Vector2.dst(getX(),getY(),u.getX(),u.getY());
    }
    ...
    public void hurt(int damage){
        if(preHurt())
        {
            ...
            health-=DamageDealt;
            CombatText.create(Integer.toString(DamageDealt), getX() + MathUtils.random(-50, 50), getY() + MathUtils.random(30, 60));
            if(health<=0) {
                preKill();
                Kill();
            }
    ...
    public BaseUnit findTarget(TYPE type){
            for (BaseUnit u : owner.enemy.getArmy()) {
                if (u!=null && u.active && u.getType() == type && distance(u) < AttackRange) {
